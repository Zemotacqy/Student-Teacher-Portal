...
public class Add_teacher{
	...
	JButton add,reset;
	Add_teacher(String value){
		f = new JFrame("Add Teacher Information");
			...
			public void actionPerformed(ActionEvent event){
				if(t1.getText().equals("") || t2.getText().equals("") || t3.getText().equals("") || t4.getText().equals("") || t5.getText().equals("")){
					...
					try{
						...
						PreparedStatement st = con.prepareStatement("insert into teacher(teacher_name,age,subject,qualifications,Contact) values(?,?,?,?,?)");
						st.setString(1,val1);
						st.setString(2,val2);
						st.setString(3,val3);
						st.setString(4,val4);
